const sm = require("./ca0109StateMachine.service"); const logger = require("../../config/logger");
...
const transitionWorkflowRecord = async (record, { flowName, toStatus, notes, userId }) => {
  ...
  logger.info({ flowName, recordId: cur.id, fromStatus: cur.status, toStatus: upd.status }, "CA-01-09: transición validada.");
  return upd;
};
