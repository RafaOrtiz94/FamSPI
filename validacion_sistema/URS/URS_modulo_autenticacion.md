...
El modulo administra el acceso al sistema mediante autenticacion con Google OAuth2, emision y renovacion de tokens JWT, cierre de sesion, consulta de identidad del usuario autenticado, aceptacion interna de LOPDP y consulta de auditoria de sesiones.
...
- Validacion de dominio corporativo permitido.
- Alta/actualizacion de usuario al autenticar.
- Emision de `accessToken` y `refreshToken`.
- Renovacion de sesion por refresh token.
- Cierre de sesion y finalizacion de token activo.
- Consulta de usuario autenticado (`me`).
- Registro de aceptacion interna LOPDP con evidencia.
- Consulta de sesiones y usuarios activos por roles autorizados.
- Deteccion de login fuera de horario y generacion de alerta.
