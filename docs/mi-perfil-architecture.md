...
### Validación contra el estado actual de la BD (evitar duplicados)
- No existe otra tabla de perfil en las migraciones actuales; `user_profile` es la única tabla nueva ligada a `users` y usa `user_id UNIQUE` para mantener normalización 1:1 sin clonar campos del IdP.
- Para asegurar que las nuevas tablas no choquen con esquemas previos, ejecutar antes de migrar:
  ```sql
  SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_name IN ('notifications', 'user_profile');
  SELECT indexname, indexdef FROM pg_indexes WHERE tablename IN ('notifications', 'user_profile');
  ```
  Si alguna existe, validar que las columnas coincidan con las definidas arriba y omitir la creación duplicada.
- Las migraciones usan `CREATE TABLE IF NOT EXISTS` e índices con `IF NOT EXISTS` para ser idempotentes en entornos donde ya se hubiese creado una versión temprana.
