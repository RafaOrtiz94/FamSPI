  ...
  const loadRequests = async () => {
    ...
    try {
      ...
      const allowed = data.filter((item) =>
        ["aprobada", "en_proceso"].includes(item.status)
      );
      setRequests(allowed);
    } catch (error) {
      console.error("Error cargando solicitudes:", error);
      toast.error("Error al cargar solicitudes aprobadas");
    } finally {
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      if (selectedCollaboratorId) {
        ...
        await updatePersonnelRequestProfile(selectedRequest.id, {
          profile: profileData,
        });
      } else {
        return;
              ...
              <div className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
                {!isRequestContext && !isCollaboratorContext ? (
                  ...
                  <div className="rounded-lg border border-yellow-200 bg-yellow-50 p-4 text-yellow-800">
                    El workspace solo está disponible cuando la solicitud está aprobada.
                  </div>
                ) : profileLoading ? (
