...
import { FiX, FiUpload, FiCalendar, FiTruck, FiFileText, FiAlertTriangle } from 'react-icons/fi';
...
const PurchaseActionBar = ({
  ...
  const getAvailableActions = () => {
    ...
    if (role.includes('gerencia') || role.includes('gerencia_general')) {
      if (status === 'pending_contract_approval') {
        actions.push({
          id: 'upload_contract',
          ...
          tooltip: 'Subir el contrato aprobado'
        });
        actions.push({
          id: 'reject',
          label: 'Rechazar',
...
// Asesor Comercial
    if (role.includes('asesor') || role.includes('comercial')) {
      if (status === 'delivery_dates_requested') {
        actions.push({
          id: 'submit_dates',
