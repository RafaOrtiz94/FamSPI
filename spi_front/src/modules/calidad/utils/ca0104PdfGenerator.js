...
export const generateCa0104Pdf = async ({ flowName, record, user }) => {
  ...
  cursorY -= 28;
  page.drawText("ACTA DE CONTROL DE PLAGAS - CA-01-04", {
    x: 50,
    y: cursorY,
    size: 14,
    font: fontBold,
  ...
  cursorY -= 20;
  page.drawText(`Flujo: ${String(flowName || "traps_map").toUpperCase()}`, {
    x: 50,
    y: cursorY,
    size: 10,
    font,
  ...
  link.href = pdfUrl;
  link.download = `CA0104_ACTA_${String(record?.id || "registro").slice(0, 8)}.pdf`;
