...
const CA0108AuthModal = ({ isOpen, onClose, onAuthenticated, actionLabel }) => {
  ...
  const handleVerify = async (event) => {
    ...
    try {
      ...
      onAuthenticated();
      onClose();
    } catch (error) { setErrorDesc(error.message || "Firma electrónica inválida."); }
    finally { setLoading(false); }
  };
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm">
      <div className="relative w-[420px] rounded-2xl border border-cyan-500/20 bg-slate-950 p-8 shadow-[0_0_50px_rgba(6,182,212,0.15)]">
        ...
        <div className="mb-6 flex flex-col items-center">
          ...
          <p className="mt-1 text-center text-sm text-slate-400">Confirma tu identidad para {actionLabel || "validar la contingencia"}</p>
        </div>
        <form onSubmit={handleVerify}>
          <div className="mb-4">
            <label className="mb-2 block text-sm font-medium text-slate-300">Token 2FA</label>
            <input type="password" value={token} onChange={(e) => setToken(e.target.value)}
              className="w-full rounded-lg border border-slate-700 bg-slate-900 px-4 py-3 text-white focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20" placeholder="Ingresa tu token..." autoFocus />
          </div>
          ...
          <button type="submit" disabled={loading || token.trim().length < 4}
            className="w-full rounded-lg bg-cyan-600 py-3 font-medium text-white hover:bg-cyan-500 disabled:cursor-not-allowed disabled:opacity-50">
...
export default CA0108AuthModal;