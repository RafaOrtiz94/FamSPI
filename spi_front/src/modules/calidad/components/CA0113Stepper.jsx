...
import { FiCheck, FiMail, FiSend, FiArchive, FiX } from "react-icons/fi";

const steps = [
  { key: "draft", label: "Borrador", icon: FiMail },
  { key: "pending", label: "Pendiente", icon: FiSend },
  { key: "approved", label: "Aprobado", icon: FiCheck },
  { key: "published", label: "Publicado", icon: FiArchive }
];

export default function CA0113Stepper({ record, onClose }) {
  const [notes, setNotes] = useState("");
  const currentStatus = useMemo(() => record?.status || "draft", [record]);
  const activeStepIndex = useMemo(() => steps.findIndex(s => s.key === currentStatus), [currentStatus]);

  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200">
        <h3 className="font-semibold text-gray-900">Flujo de Comunicación</h3>
      ...
      <div className="flex-1 p-4">
        ...
        <div className="bg-gray-50 rounded-lg p-4 mb-4">
          <h4 className="text-sm font-medium text-gray-700 mb-2">Información de Comunicación</h4>
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div><span className="text-gray-500">Tipo:</span><span className="ml-2 font-medium">{record?.communication_type || "N/A"}</span></div>
            <div><span className="text-gray-500">Prioridad:</span><span className="ml-2 font-medium capitalize">{record?.priority || "N/A"}</span></div>
            <div><span className="text-gray-500">Título:</span><span className="ml-2 font-medium">{record?.title || "N/A"}</span></div>
            <div><span className="text-gray-500">Canal:</span><span className="ml-2 font-medium">{record?.channel || "N/A"}</span></div>
          </div>
        </div>
        <div className="mb-4">
