...
import { FiArrowRight, FiCheckCircle, FiFileText, FiLock, FiShield, FiTarget } from "react-icons/fi";
import { useTransitionPestControlRecord } from "../hooks/useCa0104Queries";
import CA0104AuthModal from "./CA0104AuthModal";
import { generateCa0104Pdf } from "../utils/ca0104PdfGenerator";
...
const CA0104Stepper = ({ flowName = "traps_map", record = {}, currentUser, onTransitionSuccess }) => {
  const [notes, setNotes] = useState("");
  const [errorDesc, setErrorDesc] = useState("");
  const [loading, setLoading] = useState(false);
  const [authOpen, setAuthOpen] = useState(false);
  const transitionRecord = useTransitionPestControlRecord();
  ...
  const FlowIcon = FLOW_LABELS[flowName] ? FiTarget : FiShield;
  ...
  const executeTransition = async () => {
    try {
      ...
      if (nextState?.id === "archived" && notes.trim().length < 3) {
        throw new Error("La observacion de cierre requiere evidencia suficiente.");
      }

      await transitionRecord.mutateAsync({
        flowName,
        recordId: record.id,
        toStatus: nextState?.id || record.status || "draft",
        qaNotes: notes || `Avance manual por ${currentUser?.name || "operador GXP"}.`,
      });
  ...
  return (
    <div className="mt-4 rounded-2xl border border-white/10 bg-slate-950/70 p-5 shadow-inner">
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-[1fr_auto] md:items-end">
        <label className="grid gap-2">
          ...
          <textarea
            ...
            placeholder="Agrega contexto operativo, QA o hallazgo."
      ...
      <div className="mt-4 flex items-center gap-2 text-xs uppercase tracking-[0.2em] text-slate-500">
        <FiLock />
        {isTerminal ? "Estado final alcanzado" : "Siguiente paso con bloqueo de integridad"}
      </div>
        ...
        <div className="mt-4 rounded-xl border border-emerald-500/20 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-200">
          <div className="flex flex-col gap-3">
            <div className="flex items-center gap-2 font-semibold">
              <FiFileText />
              Flujo cerrado y listo para archivo documental.
            </div>
            <button
              type="button"
              onClick={() => generateCa0104Pdf({ flowName, record, user: currentUser })}
