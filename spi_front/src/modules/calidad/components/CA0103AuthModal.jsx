...
const CA0103AuthModal = ({ isOpen, onClose, onAuthenticated, actionLabel }) => {
  ...
  const handleVerify = async (event) => {
    ...
    try {
      ...
      if (token.trim().length < 4) {
        throw new Error("Token 2FA insuficiente para liberar la validacion final.");
      }
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm">
      <div className="relative w-[420px] rounded-2xl border border-violet-500/20 bg-slate-950 p-8 shadow-[0_0_50px_rgba(168,85,247,0.18)]">
        <button
          type="button"
          onClick={onClose}
        ...
        <div className="mb-6 flex flex-col items-center">
          <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-full border border-violet-500/40 bg-violet-500/10 text-violet-300">
          ...
          <p className="mt-2 text-center text-xs font-semibold uppercase tracking-[0.24em] text-slate-400">
            {actionLabel || "Autorizacion de validacion final"}
          </p>
        </div>
        ...
        <form onSubmit={handleVerify}>
          <div className="space-y-4">
            ...
            <div>
              <label className="mb-2 block text-xs font-bold uppercase tracking-[0.24em] text-slate-400">
                Token 2FA o clave de seguridad
              </label>
              <input
                type="password"
                value={token}
                onChange={(event) => setToken(event.target.value)}
                className="w-full rounded-xl border border-white/10 bg-slate-900/80 px-4 py-3 text-center font-mono tracking-[0.3em] text-white outline-none transition focus:border-violet-400"
                placeholder="••••••••"
                required
              />
            </div>
          ...
          <div className="mt-8">
            ...
            >
              {loading ? "Verificando..." : "Firmar y validar"}
            </button>
...
export default CA0103AuthModal;
