...
const client = axios.create({
  baseURL: `${resolveApiOrigin()}/api/v1/ti-assets/public`,
  withCredentials: false,
  timeout: 15000,
});

export async function getPublicTiAsset(assetCode) {
  const { data } = await client.get(`/${encodeURIComponent(assetCode)}`);
